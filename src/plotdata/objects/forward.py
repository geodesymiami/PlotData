...
class Okada():
    def __init__(self):
        ...
        Parameters:
            ...
            param2: fault's angle of rupture where 0 represents a strike-slip fault and 90 represents a normal fault (degrees)
            nu: Poisson's ratio
            tilt: boolean to indicate calculation of tilt displacements (True) or 3d displacements (False) (default False)
        """
        pass

    def model(self, x, y, xtlc, ytlc, dtlc, length, width, param1=0, opening=0, strike=0.0, dip=0.0, param2=0.0, nu=0.25, opt='R', tilt=False):
        L = length
        W = width
        ...
        ux = - U1 / (2 * np.pi) * self.chinnery(self.ux_ss, x, p, L, W, q, dip, nu) - \
            U2 / (2 * np.pi) * self.chinnery(self.ux_ds, x, p, L, W, q, dip, nu) + \
            U3 / (2 * np.pi) * self.chinnery(self.ux_tf, x, p, L, W, q, dip, nu)

        uy = - U1 / (2 * np.pi) * self.chinnery(self.uy_ss, x, p, L, W, q, dip, nu) - \
            U2 / (2 * np.pi) * self.chinnery(self.uy_ds, x, p, L, W, q, dip, nu) + \
            U3 / (2 * np.pi) * self.chinnery(self.uy_tf, x, p, L, W, q, dip, nu)

        uz = - U1 / (2 * np.pi) * self.chinnery(self.uz_ss, x, p, L, W, q, dip, nu) - \
            U2 / (2 * np.pi) * self.chinnery(self.uz_ds, x, p, L, W, q, dip, nu) + \
            U3 / (2 * np.pi) * self.chinnery(self.uz_tf, x, p, L, W, q, dip, nu)
    ...
    def chinnery(self, f, x, p, L, W, q, dip, nu):
        u =  (f(x, p, q, dip, nu) -
            f(x, p - W, q, dip, nu) -
            f(x - L, p, q, dip, nu) +
            f(x - L, p - W, q, dip, nu))
    ...
    def ux_ss(self, xi, eta, q, dip, nu):

        R = np.sqrt(xi ** 2 + eta ** 2 + q ** 2)
        u = xi * q / (R * (R + eta)) + \
            self.I1(xi, eta, q, dip, nu, R) * np.sin(dip)
        k = (q != 0)
        u[k] = u[k] + np.arctan( (xi[k] * eta[k]) / (q[k] * R[k]) )
        return u

    def uy_ss(self, xi, eta, q, dip, nu):
        R = np.sqrt(xi ** 2 + eta ** 2 + q ** 2)
        u = (eta * np.cos(dip) + q * np.sin(dip)) * q / (R * (R + eta)) + \
            q * np.cos(dip) / (R + eta) + \
            self.I2(eta, q, dip, nu, R) * np.sin(dip)
        return u

    def uz_ss(self, xi, eta, q, dip, nu):
        R = np.sqrt(xi ** 2 + eta ** 2 + q ** 2)
        db = eta * np.sin(dip) - q * np.cos(dip)
        u = (eta * np.sin(dip) - q * np.cos(dip)) * q / (R * (R + eta)) + \
            q * np.sin(dip) / (R + eta) + \
            self.I4(db, eta, q, dip, nu, R) * np.sin(dip)
        return u

    def ux_ds(self, xi, eta, q, dip, nu):
        R = np.sqrt(xi ** 2 + eta ** 2 + q ** 2)
        u = q / R - \
            self.I3(eta, q, dip, nu, R) * np.sin(dip) * np.cos(dip)
        return u

    def uy_ds(self, xi, eta, q, dip, nu):
        R = np.sqrt(xi ** 2 + eta ** 2 + q ** 2)
        u = ( (eta * np.cos(dip) + q * np.sin(dip)) * q / (R * (R + xi)) -
            self.I1(xi, eta, q, dip, nu, R) * np.sin(dip) * np.cos(dip) )
        k = (q != 0)
        u[k] = u[k] + np.cos(dip) * np.arctan( (xi[k] * eta[k]) / (q[k] * R[k]))
        return u

    def uz_ds(self, xi, eta, q, dip, nu):
        R = np.sqrt(xi ** 2 + eta ** 2 + q ** 2)
        db = eta * np.sin(dip) - q * np.cos(dip)
        u = ( db * q / (R * (R + xi)) -
            self.I5(xi, eta, q, dip, nu, R, db) * np.sin(dip) * np.cos(dip) )
        k = (q != 0)
        #u[k] = u[k] + np.sin(dip) * np.arctan2(xi[k] * eta[k] , q[k] * R[k])
        u[k] = u[k] + np.sin(dip) * np.arctan( (xi[k] * eta[k]) / (q[k] * R[k]))
        return u

    def ux_tf(self, xi, eta, q, dip, nu):
        R = np.sqrt(xi**2 + eta**2 + q**2)
        u = q**2 / (R * (R + eta)) - \
            (self.I3(eta, q, dip, nu, R) * np.sin(dip)**2)
        return u

    def uy_tf(self, xi, eta, q, dip, nu):
        R = np.sqrt(xi**2 + eta**2 + q**2)
        u = - (eta * np.sin(dip) - q * np.cos(dip)) * q / (R * (R + xi)) - \
            (np.sin(dip) * xi * q / (R * (R + eta))) - \
            (self.I1(xi, eta, q, dip, nu, R) * np.sin(dip) ** 2)
        k = (q != 0)
        #u[k] = u[k] + np.sin(dip) * np.arctan2(xi[k] * eta[k] , q[k] * R[k])
        u[k] = u[k] + np.sin(dip) * np.arctan( (xi[k] * eta[k]) / (q[k] * R[k]) )
        return u

    def uz_tf(self, xi, eta, q, dip, nu):
        R = np.sqrt(xi**2 + eta**2 + q**2)
        db = eta * np.sin(dip) - q * np.cos(dip)
        u = (eta * np.cos(dip) + q * np.sin(dip)) * q / (R * (R + xi)) + \
            np.cos(dip) * xi * q / (R * (R + eta)) - \
            self.I5(xi, eta, q, dip, nu, R, db) * np.sin(dip)**2
        k = (q != 0)
        u[k] = u[k] - np.cos(dip) * np.arctan( (xi[k] * eta[k]) / (q[k] * R[k]) )
        return u

    def I1(self,xi, eta, q, dip, nu, R):
        db = eta * np.sin(dip) - q * np.cos(dip)
        if np.cos(dip) > eps:
            I = (1 - 2 * nu) * (- xi / (np.cos(dip) * (R + db))) - \
                np.sin(dip) / np.cos(dip) * self.I5(xi, eta, q, dip, nu, R, db)
        else:
            I = -(1 - 2 * nu)/2 * xi * q / (R + db)**2
        return I

    def I2(self, eta, q, dip, nu, R):
        I = (1 - 2 * nu) * (-np.log(R + eta)) - \
            self.I3(eta, q, dip, nu, R)
        return I

    def I3(self, eta, q, dip, nu, R):
        yb = eta * np.cos(dip) + q * np.sin(dip)
        db = eta * np.sin(dip) - q * np.cos(dip)
        if np.cos(dip) > eps:
            I = (1 - 2 * nu) * (yb / (np.cos(dip) * (R + db)) - np.log(R + eta)) + \
                np.sin(dip) / np.cos(dip) * self.I4(db, eta, q, dip, nu, R)
        else:
            I = (1 - 2 * nu) / 2 * (eta / (R + db) + yb * q / (R + db) ** 2 - np.log(R + eta))
        return I


    def I4(self, db, eta, q, dip, nu, R):
        if np.cos(dip) > eps:
    ...
    def I5(self, xi, eta, q, dip, nu, R, db):
        X = np.sqrt(xi**2 + q**2)
        if np.cos(dip) > eps:
...
class McTigue():
    """
    A class used to represent a point source using the McTigue (1987) model.
    ...
    def model(self, x, y, xcen,ycen,depth,radius,dP_mu, nu=0.25):
        """
        3d displacement field on surface from point source (McTigue, 1987)

        Parameters
        ----------
        x : array
            x-coordinate for displacement (m)
        y : array
            y-coordinate for displacement (m)
        xcen : float
            x-offset of point source epicenter (m)
        ycen : float
            y-offset of point source epicenter (m)
        depth : float
            depth to point (m)
        radius : float
...
class Penny():
    """
    A class used to represent a point source using the Fialko (2001) model.
    ...
    def model(self, x, y, xcen, ycen, depth, radius, dP_mu, nu=0.25):
        """
        3d displacement field on surface from point source (Fialko, 2001)
        ...
        -------
        Ue : array
            displacements in east in meters.
        Un : array
            displacements in north in meters.
        Uz : array
            displacements in vertical in meters.
        """
        ...
        fi, psi, t, Wt = self.FREDHOLM(h, nis, eps)
        ...
        for t_i in range(0, np.size(t)):
            ...
            Ur_app , U3_app = self.INTGR(r,fi_sub,psi_sub,h,Wt_sub,t_sub)
    ...
    def FPKERNEL(self,h,t,r,n):
        ...
        if n == 1:
            K = p*h*(self.KG(t-r,p)-self.KG(t+r,p))
        elif n == 2:
            Dlt = 1e-6
            a = t + r
            b = t - r
            ...
            if t<Dlt:
                ...
                for j in range(0, nr):
                    if r[j] > Dlt:
                        ...
                        trbl[j] = -4*h/(p + t**2)
            KERN_bp = self.KERN(b,p)
            KERN_ap = self.KERN(a,p)
            ...
            a = p/2*(c*self.KG(c,p)+d*self.KG(d,p))
            K = b - a
        elif n == 4:
            a = ((p + y)*(p + z))**2
            c = t + r
    ...
    def FREDHOLM(self, h, m, er):
        ...
        while res > er:
            for i in range(0, m*NumLegendreTerms):
                ti = t[i]
                fpkern1 = self.FPKERNEL(h,ti,t,1)
                fpkern2 = self.FPKERNEL(h,ti,t,2)
                fpkern3 = self.FPKERNEL(h,ti,t,3)
                fpkern4 = self.FPKERNEL(h,ti,t,4)
    ...
    def INTGR(self, r, fi, psi, h, Wt, t):
...
class Yang():
    """
    A class used to represent a point source using the Yang et al. (1988) model.
    """
    def __init__(self):
        pass

    def model(self, x, y, xcen, ycen, depth, s_axis_max, ratio, dP_mu, strike, dip, nu=0.25):
        """
        3d displacement field on surface from point source (Yang et al., 1988)
        ...
        depth : float
            depth to point (m)
        s_axis_max : float
            maximum semi-axis of the source (m)
        ratio : float
            ratio of the semi-axes of the source (dimensionless)
        dP_mu : float"""
        ...
        PC, PD = self.pf(P,AL,AM,SI,A,B,C,PI)
        ...
        # first call with +C
        FF1,FF2,FF3,W1,W2,W3 = self.ellip(D1,D2,PD,xx1,xx2,Z,C,ANG,ZZ0,SI)
    ...
    def pf(self, P,AL,AM,SI,A,B,C,PI):
    ...
    def ellip(self,D1,D2,PD,X,Y,Z,C,ANG,ZZ0,SI):
